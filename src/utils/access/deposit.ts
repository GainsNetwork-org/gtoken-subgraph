...
import { GTOKEN_DECIMALS_BD } from '../constants';
...
export function createOrLoadDeposit(data: DepositInput, save: boolean): Deposit {
  ...
  if (deposit == null) {
    ...
    deposit.assets = assets.toBigDecimal().div(exponentToBigDecimal(vault.assetDecimals));
    deposit.shares = shares.toBigDecimal().div(GTOKEN_DECIMALS_BD);
